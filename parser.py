...
def getRoom(flat_page):
    ...
    if room_number == "":        #на некоторых страничках кол-во комнат не указано, поэтому делаем проверку
        room_number = np.nan     # np.nan имеет тип float, возможно, имело смысл писать return np.nan
    return float(room_number)   # а здесь return int(room_number) , но хотелось, чтобы все элементы колонки были
                                # одного типа


#Функция возвращает площадь комннаты, получив на вход строку, содержащуюю эту информацию
def get_sp(s):
    s = s.strip(' ')             #разбиваем строку на слова
    s = re.split('\xa0', s)      # теперь иформация о площади содержится в первом элементе списка
    if '–' in s[0]:        # но не во всех квартирах есть информация о площади всех комнат
        return np.nan
    if ',' in s[0]:        # дробная часть отделена запятой
        s[0] = s[0].replace(',', '.')   #исправляем это

    return float(s[0])

#Функция возвращает время в минутах и 1, если можно дойти пешком и 0 - в противном случае
#Если информации об этом нет, возвращет np.nan
def get_metroDist_walk(flat_page):
    w = np.nan
    metro = flat_page.find('span', attrs={'class': 'object_item_metro_comment'})
    metro = html_stripper(metro)

    if '-' in metro:              #проверка, есть ли информация о времени до метро
        return np.nan, w
    walk = re.split(' ', metro)
    if ('пешком\n' in walk):
        w = 1
...
def get_roomInfo(flat_page, flatStats):
    ...
    lst = re.split('-|\n', info)
    lst_new = []
    #Разбили весь текст на строчки
    # Тепрь отберём непустые строки
    for i in lst:
        if i != '':
            lst_new.append(i)

    #Теперь если в k-ой строке есть название характеристики(например, этаж), то в k+1 лежит информация о ней
    for i in range(len(lst_new)):
        if lst_new[i] == 'Этаж:':
            s = lst_new[i + 1]
            s = re.split('\xa0', s)
            if len(s) == 3:     # в случае, если есть информация и о этаже квартиры, и о максимальном этаже в доме
                floor = int(s[0])
                nfloor = int(s[2])
            else:
                floor = int(s[0])
                    ...
                    if lst_new[i] == 'Площадь кухни:':
                        ...
                        if lst_new[i] == 'Телефон:':
                            ...
                            if lst_new[i] == 'Тип дома:':
                                ...
                                s = lst_new[i + 1].strip(', ')
                                flatStats['New'] = new[s]
                                #В i+2 строке содержится информация о типе дома
                                if ('монолитный' in lst_new[i + 2]) or ('кирпичн') in lst_new[i + 2]:
                                    flatStats['Brick'] = 1
                                if 'панельный' in lst_new[i + 2] or 'сталинский' in lst_new[i + 2] \
                                        or 'блочный' in lst_new[i + 2] or 'деревянный' in lst_new[i + 2]:
                                    flatStats['Brick'] = 0
...
for j in range(len(districts)):
    ...
    for i in range(len(links)):
        ...
        #Инициализируем пустой словарь, чтобы в случае, если мы не найдём какой-то информации по квартире, это поле
        #было заполнено np.nan
